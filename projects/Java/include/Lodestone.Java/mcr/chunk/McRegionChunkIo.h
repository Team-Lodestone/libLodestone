...
namespace lodestone::java::mcr::chunk {
    // TODO we can probably make all of these take an Options class and read that
    class McRegionChunkIO : public io::chunk::NbtChunkIO {
    public:
        static constexpr int CHUNK_WIDTH = 16;
        static constexpr int CHUNK_DEPTH = 16;
        static constexpr int CHUNK_HEIGHT = 128;

        std::unique_ptr<level::chunk::Chunk> read(nbt::tag_compound &chunk, int version) const override;

        nbt::tag_compound write(level::chunk::Chunk *c, const level::types::Vec2i &coords, int version) const override;
        ...
        void write(lodestone::level::chunk::Chunk *c,const level::types::Vec2i &coords, int version, std::ostream &out) const override;
    };
}
