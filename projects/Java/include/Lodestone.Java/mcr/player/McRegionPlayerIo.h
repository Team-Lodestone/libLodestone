...
namespace lodestone::java::mcr::player {
    ...
    public:
        ...
        std::unique_ptr<lodestone::level::entity::Player> read(const std::filesystem::path &filename, std::istream &in,
            int version) const override;

        void write(lodestone::level::entity::Player *p, int version, std::ostream &out) const override;
    };
}
