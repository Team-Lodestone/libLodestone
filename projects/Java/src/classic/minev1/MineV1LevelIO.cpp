...
namespace lodestone::java::classic::minev1 {
            ...
            for (int z = 0; z < DEPTH; z++) {
                for (int x = 0; x < WIDTH; x++) {
                    ...
                    level::block::properties::BlockProperties b = io->convertBlockToInternal(
                        lodestone::conversion::block::data::ClassicBlockData(bb));
    ...
    void MineV1LevelIO::write(lodestone::level::Level *l, const int version, std::ostream &out) const {
        ...
        for (int y = 0; y < HEIGHT; y++) {
            for (int z = 0; z < DEPTH; z++) {
                for (int x = 0; x < WIDTH; x++) {
                    const level::block::properties::BlockProperties *b = l->getBlock(x, y, z);
