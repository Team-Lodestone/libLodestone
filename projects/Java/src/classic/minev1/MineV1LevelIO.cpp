        ...
        for (int y = 0; y < HEIGHT; y++) {
            for (int z = 0; z < DEPTH; z++) {
                for (int x = 0; x < WIDTH; x++) {
                    level::block::state::BlockState b = io->convertBlockToInternal(new level::conversion::block::data::ClassicBlockData(*data));
            ...
            for (int y = 0; y < HEIGHT; y++) {
                for (int z = 0; z < DEPTH; z++) {
                    ...
                    if (b->getBlock() != level::block::BlockRegistry::sDefaultBlock) {
                        ...
                        out[INDEX_YZX(x, y, z, WIDTH, DEPTH)] = (bd->as<level::conversion::block::data::ClassicBlockData>())->getId();
                    }
                }
            }
        }
