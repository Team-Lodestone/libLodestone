...
namespace lodestone::level::types {
    ...
    struct Vec2 final : StringSerializable {
        ...
        constexpr Vec2 operator+(const T v) const {
            return {
                x + v,
                z + v
            };
...
template<typename T>
struct std::hash<lodestone::level::types::Vec2<T>> {
    size_t operator()(const lodestone::level::types::Vec2<T> &v) const noexcept {
