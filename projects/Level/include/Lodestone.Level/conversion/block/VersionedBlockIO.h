...
namespace lodestone::level::conversion::block::version {
    ...
    public:
        constexpr void registerBlock(const uint32_t version, const registry::NamespacedString *internal, data::AbstractBlockData* blk) {
            mFromInternalConversionMap[version][internal] = blk;
        }

        std::unique_ptr<BlockIO> getIo(uint32_t version);
    private:
        std::map<uint32_t, gtl::flat_hash_map<const registry::NamespacedString *, data::AbstractBlockData*>> mFromInternalConversionMap{};
    };
}
