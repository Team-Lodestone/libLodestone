...
namespace lodestone::level::item {
    class ItemProperties : public common::string::StringSerializable {
      public:
        ...
        ItemProperties(const common::registry::Identifier *id)
            : mItem(ItemRegistry::getInstance().getItem(id)) {};
        ...
        bool operator==(const ItemProperties &b) const {
            return mItem == b.mItem && mProperties.has_value()
                       ? mProperties == b.mProperties
                       : mProperties.has_value() == b.mProperties.has_value();
        }

        constexpr std::string toString() const override {
            return std::format("ItemProperties[block={}]",
                               mItem->getID()->getString());
        };
