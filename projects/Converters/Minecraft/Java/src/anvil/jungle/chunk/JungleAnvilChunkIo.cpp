...
namespace lodestone::minecraft::java::anvil::jungle::chunk {
    std::unique_ptr<level::chunk::Chunk> JungleAnvilChunkIO::read(nbt::tag_compound &chunk, const int version) const {
        ...
        const nbt::tag_list sections = chunk["Sections"].get().as<nbt::tag_list>();

        std::unique_ptr<JungleAnvilChunk> c = std::make_unique<JungleAnvilChunk>(
        ...
        for (auto &s : sections) {
            ...
            const int8_t *blocks = section["Blocks"].get().as<nbt::tag_byte_array>().get().data();
            const int8_t *data = section["Data"].get().as<nbt::tag_byte_array>().get().data();
            const int8_t *add = nullptr;

            if (section.has_key("Add"))
                add = section["Add"].get().as<nbt::tag_byte_array>().get().data();
                ...
                for (int cz = 0; cz < CHUNK_DEPTH; cz++) {
                    for (int cx = 0; cx < CHUNK_WIDTH; cx++) {
                        ...
                        if (b.getBlock() !=
                            ...
                            c->JungleAnvilChunk::setBlock(std::move(b), cx, lsy + cy, cz);
                    }
                }
            }
        }
    ...
    nbt::tag_compound JungleAnvilChunkIO::write(level::chunk::Chunk *c, const level::types::Vec2i &coords,
        int version) const {
        // TODO!
    }

    size_t JungleAnvilChunkIO::getSize(level::chunk::Chunk *c, int version) const {
        return 0;
    }

    std::unique_ptr<lodestone::level::chunk::Chunk> JungleAnvilChunkIO::read(std::istream &in, const int version) const {
    ...
    void JungleAnvilChunkIO::write(lodestone::level::chunk::Chunk *c, const level::types::Vec2i &coords, int version,
