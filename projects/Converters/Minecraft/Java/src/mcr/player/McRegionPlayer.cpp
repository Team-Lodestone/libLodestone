...
namespace lodestone::java::mcr::player {
    McRegionPlayer::McRegionPlayer(const std::string &name)
        : mName(name), mSleepTimer(0), mIsSleeping(false), mDeathTime(0),
          mHurtTime(0), mAttackTime(0), mFireTime(0), mBreathingTime(0),
          mDimension(level::world::World::Dimension::OVERWORLD),
          mFallDistance(0) {}

    const std::string &McRegionPlayer::getId() const { return mName; }
    ...
    short McRegionPlayer::getDeathTime() const { return mDeathTime; }

    void McRegionPlayer::setDeathTime(const short deathTime) {
        mDeathTime = deathTime;
    }

    short McRegionPlayer::getHurtTime() const { return mHurtTime; }

    void McRegionPlayer::setHurtTime(const short hurtTime) {
        mHurtTime = hurtTime;
    }

    short McRegionPlayer::getAttackTime() const { return mAttackTime; }

    void McRegionPlayer::setAttackTime(const short attackTime) {
        mAttackTime = attackTime;
    }

    short McRegionPlayer::getFireTime() const { return mFireTime; }

    void McRegionPlayer::setFireTime(const short fireTime) {
        mFireTime = fireTime;
    }

    short McRegionPlayer::getBreathingTime() const { return mBreathingTime; }
    ...
    void McRegionPlayer::setDimension(
        const common::registry::Identifier &dimension) {
    ...
    float McRegionPlayer::getFallDistance() const { return mFallDistance; }
    ...
    McRegionPlayer::getInventory() const {
        return mInventory;
    }
