...
namespace lodestone::minecraft::java::mcr::world {
    ...
    int64_t McRegionWorld::getSeed() const { return mSeed; }

    void McRegionWorld::setSeed(const int64_t seed) { mSeed = seed; }
    ...
    int64_t McRegionWorld::getLastPlayed() const { return mLastPlayed; }

    void McRegionWorld::setLastPlayed(const int64_t lastPlayed) {
        mLastPlayed = lastPlayed;
    }

    int64_t McRegionWorld::getSize() const { return mSize; }

    void McRegionWorld::setSize(const int64_t size) { mSize = size; }

    int64_t McRegionWorld::getTime() const { return mTime; }

    void McRegionWorld::setTime(const int64_t time) { mTime = time; }
