...
namespace lodestone::level::types {
    ...
    struct LODESTONE_API Bounds3 final
        ...
        constexpr Bounds3(const Vec3<T> &min, const Vec3<T> &max)
            : min(min), max(max) {};

        constexpr Bounds3(const int x0, const int y0, const int z0,
        ...
        constexpr bool contains(const Vec3<T> &vec) const noexcept {
            return vec.x >= min.x && vec.x <= max.x && vec.y >= min.y &&
                   vec.y <= max.y && vec.z >= min.z && vec.z <= max.z;
        }
