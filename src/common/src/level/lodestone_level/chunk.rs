...
impl Chunk {
    ...
    pub fn generate_blockmap(&self) -> Vec<u16> {
        ...
        for z in 0..16 {
            for x in 0..16 {
                for y in (0..self.height).rev() {
                    let blk = self.get_block(x, y, z);
                    if (!crate::block::get_block(blk).expect("Get block for blockmap").is_translucent_map) {
                        blkmap.push(blk);
                        break;
                    }
                }
    ...
    pub fn set_block(&self, x: i8, y: i16, z: i8, block: u16) {
        if (x > 16 || y > self.height || z > 16
            || x < 0 || y < 0 || z < 0) {
            return;
        }
    ...
    pub fn set_state(&self, x: i8, y: i16, z: i8, state: u8) {
        if (x > 16 || y > self.height || z > 16
            || x < 0 || y < 0 || z < 0) {
            return;
        }
    ...
    pub fn set_light(&self, light_type: Light, x: i8, y: i16, z: i8, mut level: u8) {
        if (x > 16 || y > self.height || z > 16
            || x < 0 || y < 0 || z < 0) {
            return;
        }
